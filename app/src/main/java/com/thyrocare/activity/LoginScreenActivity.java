...
public class LoginScreenActivity extends AbstractActivity implements View.OnClickListener {
    ...
    */
    ...
    public void onClick(View v) {
        if (v.getId() == R.id.forget_password) {
            ...
            if (validate()) {
                ...
                loginRequestModel.setUserName(edt_username_login.getText().toString().trim()+"|"+DeviceUtils.getDeviceId(activity));
    ...
    private void registerChat() {
        ...
        RequestQueue rQueue = Global.setVolleyReq(LoginScreenActivity.this);
        rQueue.add(request);
    }
        ...
        try {
            if (!InputUtils.isNull(loginResponseModel.getUserID())) {
               ...
               /* Global global = new Global(activity);
                device_id = global.getDeviceIMEI(activity);*/
