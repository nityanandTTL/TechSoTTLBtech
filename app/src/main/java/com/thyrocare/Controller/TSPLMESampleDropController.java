...
public class TSPLMESampleDropController {
    ...
    public void CallGetDateWiseWLMIS(String Id) {
        ...
        ApiCallAsyncTask sampleDropDetailsbyTSPLME = new AsyncTaskForRequest(mContext).getWLMISDateWise(Id);
        sampleDropDetailsbyTSPLME.setApiCallAsyncTaskDelegate(new DateWiseWLMISAsyncTaskDelegateResult());
        if (isNetworkAvailable(mContext)) {
            sampleDropDetailsbyTSPLME.execute(sampleDropDetailsbyTSPLME);
        } else {
            Toast.makeText(mContext, R.string.internet_connetion_error, Toast.LENGTH_SHORT).show();
    ...
    private class DateWiseWLMISAsyncTaskDelegateResult implements ApiCallAsyncTaskDelegate {
        @Override
        public void apiCallResult(String json, int statusCode) throws JSONException {
            if (statusCode == 200) {

                ResponseParser responseParser = new ResponseParser(mContext);
                ArrayList<DateWiseWLMISDetailsModel> materialDetailsModels = new ArrayList<>();
                materialDetailsModels = responseParser.getDateWiseWLMISResponseModel(json, statusCode);
                try {
                    if (materialDetailsModels != null) {
                        if (materialDetailsModels.size() != 0) {
                ...
                Toast.makeText(mLMEMapDisplayFragmentActivity, "" + json, Toast.LENGTH_SHORT).show();
            }
        }
